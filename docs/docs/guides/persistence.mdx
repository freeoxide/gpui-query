...
:::tip
Sanitize errors before they reach persistence. A `QueryError` built from a raw server response can contain tokens or connection strings; once it is on a resource it can flow into a dehydrate snapshot. See [Error handling — Error sanitization](/guides/error-handling#error-sanitization).
:::
