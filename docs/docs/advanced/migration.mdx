...
And use `QueryError::sanitized()` at the boundary where you convert server responses, so tokens and connection strings do not leak into logs or [DevTools](/advanced/devtools).
...
- [API reference](/advanced/api-reference) — the flat index of v2 types and functions.
- [Comparison: vs. raw async](/advanced/comparison) — what v2 removes versus hand-rolled `cx.spawn`.
- [Quick start](/getting-started/quick-start) — the canonical v2 entry point.
