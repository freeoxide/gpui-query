...
Mutations handle write operations: creating a user, updating a record, deleting a resource. Unlike [Queries](/api/queries), mutations don't run automatically. You trigger them explicitly, and they manage their own loading/success/failure lifecycle.
...
Shared between queries and mutations. See the [Queries](/api/queries) page for the full reference. The defaults for mutations differ: `use_mutation` creates a resource with `RetryPolicy::no_retries()`, while `MutationOptions::new()` also defaults to no retries.
