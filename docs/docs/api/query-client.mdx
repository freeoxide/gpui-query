...
`QueryClient` is a GPUI [`Global`] that holds every [query](/api/queries), infinite query, and [mutation](/api/mutations) entity in your application. You set it up once during app initialization and access it from any GPUI context through `cx.global::<QueryClient>()`.
...
`QueryClient::new` takes two required arguments: a default `CachePolicy` and a default `RequestPolicy`. Every [query resource](/api/queries) created through `resource()` inherits these unless you override them per-resource.
