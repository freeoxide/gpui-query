...
It assumes you have already [installed gpui-query and set up a `QueryClient` global](/getting-started/installation).
...
- `use_query` — a hook you call from a view's constructor (not in `render`). It fetches the first time, subscribes the view to the result, and returns an [`Entity`](/api/queries) you read during render.
...
- [Queries](/api/queries) — the full `use_query` surface, the fetcher signal, and refetching.
- [Mutations](/api/mutations) — lifecycle callbacks and the `mutate` variants.
- [Caching](/guides/caching) — `CachePolicy`, request deduplication, and GC.
- [Query keys](/guides/query-keys) — hierarchical keys and invalidation scope.
