...
sidebar_position: 2
title: Devtools
description: "Inspect live query and mutation state in gpui-query: ClientDiagnostic snapshots, dehydrate/hydrate, and patterns for building a debug panel."
---

gpui-query exposes runtime diagnostics through the [QueryClient](/docs/api/query-client). You get a serializable snapshot of every active query and mutation, which you can log, render in a debug panel, or serialize to JSON for later analysis.
...
|---|---|
| `key` | The stringified [QueryKey](/docs/api/queries) for this resource. |
...
After calling `invalidate_queries` on the [QueryClient](/docs/api/query-client), run diagnostics again to confirm the target resources moved to the expected status. If a key is still `"Success"` with stale data, your invalidation filter did not match it.
