...
Observers bridge gpui-query's resources to GPUI's reactivity. An observer attaches to a resource entity and calls `cx.notify()` to trigger a re-render, but only when the resource's status changes. This is the primitive that keeps `use_query`, `use_infinite_query`, and `use_mutation` from re-rendering your view on every internal state mutation.
...
- [Queries](/docs/api/queries) / [Infinite queries](/docs/api/infinite-queries) / [Mutations](/docs/api/mutations): the hooks that build observers for you.
- [DevTools](/docs/advanced/devtools): diagnostics read the same live entities observers watch.
- [Error handling](/docs/guides/error-handling): why `Cancelled` rarely reaches your UI (it is a status, deduplicated like any other).
