...
The bulk operations on `QueryClient` do not take a raw key — they take a [`QueryKeyFilter`](/docs/api/query-client), which is how you say *which* keys to act on. There are three variants:
...
- [`QueryClient`](/docs/api/query-client) — the bulk operations that consume a `QueryKeyFilter`.
- [Caching](/docs/guides/caching) — how `CachePolicy` interacts with keys.
- [Error handling](/docs/guides/error-handling) — surfacing failures keyed by resource.
