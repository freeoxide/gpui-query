...
description: "Handle query errors in gpui-query: the error status lifecycle, retries with backoff, cooperative cancellation, and rendering failures in GPUI views."
---
...
Every query and mutation uses [`QueryError`](/docs/api/queries) as the default error type. It wraps two fields: a [`QueryErrorKind`](/docs/api/queries) and a message string.
...
When a query fails, the error is stored on the [`QueryResource`](/docs/api/queries). Read it inside `read_with`:
...
[QueryStatus](/docs/api/queries) has three states relevant to errors:
...
Mutations also track a retry count. Call `should_retry()` to check whether the retry policy allows another attempt, then call `retry()` to transition back to `Loading`. See [Retry](/docs/guides/retry) for the full retry configuration.
...
For retry configuration, see the [Retry](/docs/guides/retry) guide. For the full query and mutation API, see [Queries](/docs/api/queries) and [Mutations](/docs/api/mutations).
