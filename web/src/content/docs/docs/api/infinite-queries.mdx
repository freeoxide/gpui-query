...
On creation, the hook starts an initial fetch if the resource is idle. The entity is registered with [QueryClient](/docs/api/query-client) for shared caching and garbage collection.
...
The guard proves the request was still current at acceptance time. This is the same pattern used by [Queries](/docs/api/queries) on `QueryResource`.
