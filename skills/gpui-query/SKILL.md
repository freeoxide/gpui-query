...
`use_query_with_policy` is identical except `Fut: Future<Output = Result<Fetched<T>, E>>`: the fetcher returns a `Fetched<T>` so a server-derived `CachePolicy` overrides the resource's policy on success (**"server wins"**):
...
|---|---|
| `RetryPolicy::no_retries()` | `max_retries: 0`. (A bare `QueryResource::new` starts here; the hook installs the real policy.) |
...
- exponential on → `retry_delay_ms * 2^attempt`, where the shift is clamped to 62 (prevents overflow) and the multiply is saturating; then `.min(max_retry_delay_ms).min(3_600_000)`, a **hard 1-hour ceiling** regardless of config.

`should_retry(current_retries)` = `current_retries < max_retries`. The rest-state contract is uniform across families: `retry_count` is nonzero only mid-retry-sequence, 0 after an accepted success or terminal failure, and discards never touch it. Mutations reach that state by resetting on `begin`; queries reset inside the accept guard on completion.
...
- `.with_gc_time(ms)`: `0` skips the **opportunistic** (automatic) GC sweep only; an explicit `gc(cx)` / `gc_with_time(now, cx)` still runs (clamping any value `< 1000` to `1000` during the pass).
...
```rust
client.get_query_data::<Vec<User>, MyError>(&QueryKey::from("users"), cx)           // Option<Vec<User>> (clones T)
client.with_query_data::<Vec<User>, MyError, _>(&QueryKey::from("users"), cx, |u: &Vec<User>| u.len())  // Option<R>, zero-clone
...
```rust
// fetchQuery: forced, always returns a PreparedFetch.
...
- `is_data_stale()`: staleness heuristic; `complete_current_success(request_id, data, now_ms)` / `complete_current_failure(request_id, err, now_ms)` do accept + complete in one call for manual flows.
...
- **Inclusive TTL boundary.** `is_fresh(age)` is `age <= ttl_ms` (not `<`), the opposite of HTTP `max-age`. A `Ttl { ttl_ms: 1000 }` is still fresh at exactly 1000ms. Watch for off-by-one when translating server `Cache-Control`.
...
- **Mutation GC actually runs.** `QueryClient`'s explicit `Default` sets `gc_time_ms: 300_000` (the derived `Default` would have been `0` = disabled). `with_gc_time(0)` skips the opportunistic sweep only; an explicit `gc(cx)` still sweeps, and any value `< 1000` is clamped to `1000` during a pass. Mutations are GC-eligible by completion time, not insertion time.
